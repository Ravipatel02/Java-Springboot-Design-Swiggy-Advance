...
public class TestCases {
    ...
    void testControllerLayerAnnotations(){
        ...
        assertTrue(customer.isAnnotationPresent(Controller.class) || customer.isAnnotationPresent(RestController.class), 
                () -> String.format("[Failure] %s: -> stereotype annotation is missing on controller layer ", methodName));
        assertTrue(order.isAnnotationPresent(Controller.class) || order.isAnnotationPresent(RestController.class), 
                () -> String.format("[Failure] %s: -> stereotype annotation is missing on controller layer ", methodName));
        assertTrue(restaurant.isAnnotationPresent(Controller.class) || restaurant.isAnnotationPresent(RestController.class), 
                () -> String.format("[Failure] %s: -> stereotype annotation is missing on controller layer ", methodName));
        assertTrue(delivery.isAnnotationPresent(Controller.class) || delivery.isAnnotationPresent(RestController.class), 
                () -> String.format("[Failure] %s: -> stereotype annotation is missing on controller layer ", methodName));
    ...
    void testAddCustomer() {
        ...
        assertDoesNotThrow(() -> {
            Customer addCustomer = customerService.addCustomer(newCustomer);
            assertNotNull(addCustomer, () -> String.format("[failure] %s: -> method not working as expected",methodName));
            assertEquals(newCustomer, addCustomer, () -> String.format("[failure] %s: -> method not working as expected",methodName));
